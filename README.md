...
> **Critical:** This package still depends on solver code generated by the Python NMPC package, but the workspace now provides a platform-aware guard. From the workspace root, `make run_nmpc_cpp ...` auto-checks/regenerates the solver and rebuilds this package before launch. For direct manual builds, run `python3 src/nmpc_acados_px4/ensure_solver.py --platform sim|hw` first.
...
The C++ package links against a shared library produced by the Python package's code-generation step. The recommended direct command is now:
